...
*ᴀᴅᴍɪɴs ᴏɴʟʏ:*
 ❍ /ban <kullanıcı adı>*:* . (kullanıcıyı banlar)
 /siktir <kullanıcı adı> siktir eder😅
 ❍ /sban <kullanıcı adı>*:* sɪʟᴇɴᴛʟʏ ʙᴀɴ ᴀ ᴜsᴇʀ. ᴅᴇʟᴇᴛᴇs ᴄᴏᴍᴍᴀɴᴅ, ʀᴇᴘʟɪᴇᴅ ᴍᴇssᴀɢᴇ ᴀɴᴅ ᴅᴏᴇsɴ'ᴛ ʀᴇᴘʟʏ. (ᴠɪᴀ ʜᴀɴᴅʟᴇ, ᴏʀ ʀᴇᴘʟʏ)
 ❍ /tban  <kullanıcı adı> x(ᴍ/ʜ/ᴅ)*:* ʙᴀɴs ᴀ ᴜsᴇʀ ғᴏʀ `x` ᴛɪᴍᴇ. (ᴠɪᴀ ʜᴀɴᴅʟᴇ, ᴏʀ ʀᴇᴘʟʏ). `ᴍ` = `ᴍɪɴᴜᴛᴇs`, `ʜ` = `ʜᴏᴜʀs`, `ᴅ` = `ᴅᴀʏs`.
 ❍ /unban  <kullanıcı adı>*:*  (ban kaldırır)
 ❍ /kick <ᴜsᴇʀʜᴀɴᴅʟᴇ>*:*  (kullanıcıyı tekmeker)
"""

BAN_HANDLER = CommandHandler(["ban", "siktir", "sban"], ban, run_async=True)
